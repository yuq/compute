...
static void parent(int fd)
{
	...
	hsa_agent_t dst_agent = get_agent(0);
	hsa_amd_memory_pool_t pool = get_pool(dst_agent);

	hsa_status_t status;
	void *dst_ptr;
	status = hsa_amd_memory_pool_allocate(pool, BUFF_SIZE, 0, &dst_ptr);
	...
	void *src_ptr;
	status = hsa_amd_ipc_memory_attach(&ipc, BUFF_SIZE, 0, NULL, &src_ptr);
	assert(status == HSA_STATUS_SUCCESS);

    hsa_agent_t src_agent = get_agent(1);
	hsa_agent_t import_agents[2] = {src_agent, dst_agent};
    status = hsa_amd_agents_allow_access(2, import_agents, NULL, dst_ptr);
    assert(status == HSA_STATUS_SUCCESS);

	hsa_amd_pointer_info_t info = {
		.size = sizeof(hsa_amd_pointer_info_t),
	};
	status = hsa_amd_pointer_info(src_ptr, &info, NULL, NULL, NULL);
	assert(status == HSA_STATUS_SUCCESS);
	printf("src type=%d agentbase=%p hostbase=%p own=%lu\n",
